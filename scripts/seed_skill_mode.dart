...
Future<void> main() async {
  ...
  final cards = <Map<String, dynamic>>[
    ...
    _sentence(
      ...
      grammar: {
        ...
        'note': 'dem = dative der, das = accusative das',
      },
    ),
    ...
    _sentence(
      ...
      grammar: {
        'note':
            'German word order: Time-Manner-Place (heute-mit dem Zug-nach Berlin)',
      },
    ),
    ...
    _sentence(
      ...
      grammar: {
        'note':
            'Perfect tense: auxiliary (habe) second, participle (gelesen) at end',
      },
    ),
    ...
    _sentence(
      ...
      grammar: {
        'verb': 'aufmachen',
        'note': 'Separable verbs rejoin in past participle: auf+ge+macht',
      },
    ),
  ];
  ...
  for (var i = 0; i < cards.length; i += 10) {
    final batch = cards.sublist(
      i,
      i + 10 > cards.length ? cards.length : i + 10,
    );
    ...
    if (response.statusCode >= 200 && response.statusCode < 300) {
      ...
      print(
        '  ERROR batch ${(i ~/ 10) + 1}: ${response.statusCode} ${response.body}',
      );
    }
  }
...
Map<String, dynamic> _inflected(
  ...
  return {
    ...
    'tile_config': {'root': foreign, 'suffix': '', 'suffix_color': '#10B981'},
    'sentence_tiles': null,
    'native_word_order': null,
    'foreign_word_order': null,
    'tags': tags,
